...
def create(name='', surname='', number='', email=''):
    ...
    global db_file_name
    if(name == ''):
        print("Имя небыло введенно")
        return
    if(surname == ''):
        print("Фамилия не была введена")
        return
    if(number == ''):
        print("номер телефона небыл введён")
        return
    if(email == ''):
        print("адресс почты небыл введён")
        return
...
def retrive(id='', name='', surname='', number='', email=''):
    ...
    for row in db:
        ...
        result.append(row)
    if len(result) == 0:
        return f'Контакты небыли найдены'
    else:
